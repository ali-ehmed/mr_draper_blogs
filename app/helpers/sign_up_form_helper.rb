module SignUpFormHelper
  ...
  def sign_in_modal_form_social_links(wrapper_class: nil, link_class: nil)
    tag.div(class: wrapper_class) do
      capture do
        concat(
          link_to(person_facebook_omniauth_authorize_path, method: :post, class: "btn btn-fb btn-block #{link_class}") do
            capture do
        ...
        concat(
          link_to(person_google_oauth2_omniauth_authorize_path, method: :post, class: "btn btn-google btn-block #{link_class}") do
            capture do
